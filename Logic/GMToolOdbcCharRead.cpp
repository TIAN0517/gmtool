...
int CGMToolOdbcBase::CharDataGet( int nUserNumber, int nChaNumber, SGMToolCharData* pData )
{
	...
	while (true)
	{
		...
		{		
			...
			if (cbChaName != 0 && cbChaName != -1) 
				::StringCchCopy(pCharData2->m_szName, CHR_ID_LENGTH+1, (const char*) szChaName);

			if (cbChaGuName != 0 && cbChaGuName != -1) 
				::StringCchCopy(pCharData2->m_szNick, CHAR_SZNAME+1, (const char*) szChaGuName);
			...
			if (cbChaBadge != 0 && cbChaBadge != -1) 
				::StringCchCopy(pCharData2->m_szBadge, CHAR_SZNAME+1, (const char*) szChaBadge);

			/*pk info, Juver, 2017/11/17 */
			pCharData2->m_dwPKScorePREV = (DWORD)nChaPKScore;
			pCharData2->m_dwPKDeathPREV = (DWORD)nChaPKDeath;
	...
	sReturn = m_pGameDB->ReadImage("ChaInfo.ChaCollection", nChaNumber, ByteStream);
	if (sReturn == DB_ERROR)
		return DB_ERROR;
	else
		pCharData2->SETITEMCOLLECTION_BYBUF(ByteStream);
