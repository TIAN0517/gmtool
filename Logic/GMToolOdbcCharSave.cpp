...
int CGMToolOdbcBase::CharDataSave( SGMToolCharData* pData )
{
	...
	_snprintf( szTemp, 1024, "UPDATE ChaInfo SET ChaBright=%d, ChaLevel=%u, ChaMoney=%I64d, "
		...
		pCharData2->m_dwPKScorePREV,
		pCharData2->m_dwPKDeathPREV,
	...
	pCharData2->GETITEMCOLLECTION_BYBUF(ByteStream);
	ByteStream.GetBuffer(pBuffer, dwSize);
	if (pBuffer != NULL)
	{
		TCHAR szTemp[128] = {0};
		_snprintf( szTemp, 128, "UPDATE ChaInfo SET ChaInfo.ChaCollection=? WHERE (ChaNum=%u)", dwChaNum );
		m_pGameDB->WriteImage( szTemp, dwChaNum, (BYTE *) pBuffer, dwSize );
		pBuffer = NULL;
	}	
